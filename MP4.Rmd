...
**The remakes exhibit this clearly, with some characters originally played by (or drawn as) White individuals portrayed in the remakes by people of color. For example, contrary to the original movie, the 2017 Beauty and the Beast casts a woman of color to portray Plumette.** 
...
Despite the fact that the country's population is one third of the United States, Philippines was the country  where Beauty and the Beast grossed the highest[^1]. 
...
filter(country== "Philippines") %>%
  ...
  theme(plot.title = element_text(family = "Century Gothic", size= 14), 
        legend.title = element_text(family = "Century Gothic", size = 8), 
        legend.text = element_text(family = "Century Gothic", size = 8))+ 
    theme(plot.background = element_rect(fill = "#E9DBF4"), 
          panel.grid.major = element_line("#E9DBF4"))
...
```{r warning= FALSE, message=FALSE}
# Retrieve SQL query for Emma Watson movies
emmawatson <- db %>% 
...
```{r warning= FALSE, message=FALSE}
# Retrieve SQL query for Lily James movies
...
Overall, the star power associated with these two remakes may explain the success in comparison to other movies on the market. On the level of the actors themselves, other factors contribute to the success of the movies as compared to their other works. It is interesting to note that these two movies are the only remakes done by these actors, with other movies being original productions. It is possible that audiences were not only looking for plot lines and entertainment when they headed to theatres, but also a chance to revisit timeless classics. Whatever the case, one fact is clear: Disney’s princess stories are tales that are never too old for Disney fans all around the world.**
